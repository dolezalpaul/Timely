    ...
    public sealed class ParsedScript : IDisposable
    {
        ...
        public object CallMethod(string methodName, params object[] arguments)
        {
            ...
            try
            {
                lock (_engine)
                {
                    var _currentMember = _dispatch;
                    var memberNames = methodName.Split('.');
                    for (var i = 0; i < memberNames.Length - 1; i++)
                    {
                        _currentMember = _currentMember.GetType().InvokeMember(
                            memberNames[i], BindingFlags.GetProperty, null, _currentMember, null);
                    }
                    return _currentMember.GetType().InvokeMember(
                        memberNames[memberNames.Length - 1], BindingFlags.InvokeMethod, null, _currentMember, arguments);
                }
            }
            catch
            {
